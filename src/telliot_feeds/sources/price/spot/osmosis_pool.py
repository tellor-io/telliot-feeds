...
OSMOSIS_SQS_URL = "https://sqs.osmosis.zone"
...
# Noble Dollar (USDN) and Noble USDC IBC denoms on Osmosis
USDN_IBC = "ibc/0C39BD03B5C57A1753A9B73164705871A9B549F1A5226CFD7E39BE7BF73CF8CF"
USDC_IBC = "ibc/498A0751C798A0D9A389AA3691123DADA57DAA4FE165D5C75894505B876BA6E4"


class OsmosisPoolPriceService(WebPriceService):
    """Osmosis Price Service supporting stATOM/USD and USDN/USD.

    For stATOM/USD: fetches current_sqrt_price from the Osmosis LCD REST API
    for the stATOM/ATOM concentrated liquidity pool (1283), squares it to get
    the stATOM/ATOM ratio, then multiplies by the ATOM/USD price from CoinGecko.

    For USDN/USD: queries the Osmosis SQS tokens/prices endpoint to get the
    Noble Dollar (USDN) price in Noble USDC terms. Since USDC ≈ $1, the ratio
    is returned directly as the USD price.

    Supports asset="statom" or asset="usdn", currency="usd".
    """
    ...
    async def _get_usdn_usd_price(self) -> OptionalDataPoint[float]:
        """Fetch USDN/USD price via Osmosis SQS tokens/prices endpoint.

        USDN (Noble Dollar) is priced against Noble USDC on Osmosis.
        USDC ≈ $1, so the USDN/USDC ratio is returned directly as USD price.
        """
        sqs_url = f"{OSMOSIS_SQS_URL}/tokens/prices?base={USDN_IBC}"
        try:
            with requests.Session() as s:
                r = s.get(sqs_url, timeout=self.timeout)
                if r.status_code != 200:
                    logger.warning(f"Osmosis SQS returned status {r.status_code} for USDN price")
                    return None, None
                data = r.json()
        except requests.exceptions.ConnectTimeout:
            logger.warning("Osmosis SQS request timed out for USDN price")
            return None, None
        except Exception as e:
            logger.warning(f"Osmosis SQS request failed for USDN price: {e}")
            return None, None

        prices = data.get(USDN_IBC)
        if prices is None:
            logger.warning("Osmosis SQS response missing USDN denom key")
            return None, None

        price_str = prices.get(USDC_IBC)
        if price_str is None:
            logger.warning(f"Osmosis SQS USDN response has no USDC quote. Got: {prices}")
            return None, None

        try:
            price = float(price_str)
        except (TypeError, ValueError) as e:
            logger.warning(f"Could not parse Osmosis SQS USDN price '{price_str}': {e}")
            return None, None

        if price == 0.0:
            logger.warning("Osmosis SQS returned zero price for USDN")
            return None, None

        from telliot_feeds.dtypes.datapoint import datetime_now_utc

        logger.info(f"Osmosis USDN/USD (via SQS): {price}")
    ...
    async def get_price(self, asset: str, currency: str) -> OptionalDataPoint[float]:
        ...
        if currency != "usd":
            logger.error(f"OsmosisPoolPriceService only supports usd currency, got {currency}")
            return None, None

        if asset == "usdn":
            return await self._get_usdn_usd_price()

        if asset != "statom":
            logger.error(f"OsmosisPoolPriceService only supports statom/usd or usdn/usd, got {asset}/{currency}")
            return None, None
        ...
        pool_tokens: list[dict[str, Any]] = pool.get("pool_liquidity", []) or pool.get("tokens", [])
        # CL pools expose tokens via token0/token1 or pool_assets; try all known fields
        token_denoms: set[str] = set()
...
if __name__ == "__main__":
    ...
    async def main() -> None:
        ...
        source2 = OsmosisPoolPriceSource(asset="usdn", currency="usd")
        price2, ts2 = await source2.fetch_new_datapoint()
        print(f"USDN/USD = {price2} @ {ts2}")
