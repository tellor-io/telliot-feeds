...
def build_buckets(start: int, a_list: list, bucket_size: int = TEN_MINUTES) -> dict:
    vwap_per_slot = {}
    for i in a_list:
...
def merge_as_list(from_map: dict, to_map: dict) -> list:
    merged = []
...
def volume_weighted_average_price(start: int, from_list: list, to_list: list) -> dict:
    ...
    return {
        'vwap': sum_amount_and_prices / sum_volume,
        'volume': sum_volume
    }


def retrieve_bitfinex_trades(route: str, start: int, end: int) -> list:
...
def retrieve_kucoin_trades(route: str, start: int, end: int) -> list:
    # rate-limit: 100 req/10s
    url = f'https://api.kucoin.com/api/v1/market/candles?type=1min&symbol={route}&startAt={start}&endAt={end}'
    try:
        response = requests.get(url)
    except Exception as e:
        logger.error(f'Error when retrieving Kucoin trades for {route}: HTTP/{e.response.status} - {e.response.data}')
        return []

    if not response or not response.data or len(response.data) <= 0:
        logger.warn(f'Could not get Kucoin trades for {route}')
        return []

    # transform kucoin response to bitfinex format: id, timestamp(in ms), amount, price (=average(open,close,high,low))
    transformed = []
    for row in response.data:
        price = float(row[6]) / float(row[5])
        if math.isnan(price):
            price = 0
        transformed.append(['', int(row[0]) * 1000, float(row[5]), price])
    return transformed


def calculate_bitfinex_single_via(symbol: dict, start: int, end: int, show_debug: bool) -> dict:
    ...
    if show_debug:
        result['source'] = {}
        result['source'][f'bitFinexRawData_{symbol["from"]}to{symbol["via"]}'] = build_buckets(start, from_list)
        result['source'][f'bitFinexRawData_{symbol["via"]}to{symbol["to"]}'] = build_buckets(start, to_list)
    return result


# KuCoin disabled because of too little volume
def calculate_all_single_via(symbol: dict, start: int, end: int, show_debug: bool) -> dict:
    from_list = retrieve_bitfinex_trades(symbol['hops'][0], start, end)
    to_list = retrieve_bitfinex_trades(symbol['hops'][1], start, end)
    if len(from_list) == 0 or len(to_list) == 0:
        result = {}
...
def calculate_vwap_direct(symbol_route: dict, start: int, end: int) -> dict:
...
def calculate_vwap_via(symbol: dict, start: int, end: int, show_debug: bool) -> dict:
...
def calculate_vwap_via_all(symbol: dict, start: int, end: int, show_debug: bool) -> dict:
    p_result = [
        calculate_vwap_direct(SYMBOLS[symbol['direct']], start, end)
    ]
    for h in symbol['viaHops']:
        p_result.append(calculate_all_single_via(SYMBOLS[h], start, end, show_debug))
    result = {
        'bitFinexVwapDirect': p_result[0]
    }
    ...
    all_vwaps = [v[1] for v in result.items() if v[0] != 'source' and v[1] != NO_TRADES_FOUND]
...
def get_value_from_bitfinex(symbol: dict, start: int, end: int, show_debug: bool) -> dict:
