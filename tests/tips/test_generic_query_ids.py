...
@pytest.mark.asyncio
async def test_mashup_type(autopay_contract_setup, chain, caplog):
    ...
    chain.mine(1)
    reporting_time = chain.pending_timestamp
...
@pytest.mark.asyncio
async def test_collection_type(autopay_contract_setup, chain):
...
@pytest.mark.asyncio
async def test_collection_type_build_feed(autopay_contract_setup, chain):
    ...
    query_data = HexBytes(
        encode(
            ["string", "bytes"],
            [typ, encode(["uint256", "address", "uint256"], [chain_id, contract_address, metric])],
        )
    )
    ...
    with patch("builtins.input", side_effect=[choice, chain_id, contract_address, metric]):
        ...
        chain.mine(1)
        new_reporting_time = chain.pending_timestamp
...
@pytest.mark.asyncio
async def test_mashup_type_build_feed(autopay_contract_setup, chain):
    r = autopay_contract_setup
    list_sorted = sorted(DATAFEED_BUILDER_MAPPING)
