...
async def test_ousideof_window_pt0(autopay_contract_setup):
    ...
    tip_amount = await fetch_feed_tip(autopay=contract, query_id=query_id, timestamp=chain.time() + 2)
    assert tip_amount == 0
...
async def test_onetimetip_and_feedtip(autopay_contract_setup):
    ...
    autopay, oracle = await autopay_contract_setup
    # setup and fund a feed on autopay
    _, _ = await autopay.write(
        "setupDataFeed",
        **txn_kwargs,
        **setup_datafeed_kwargs_big_window,
        _priceThreshold=price_threshold,
    ...
    tip_amount = await fetch_feed_tip(autopay=autopay, query_id=query_id, timestamp=chain.time() + 2)
