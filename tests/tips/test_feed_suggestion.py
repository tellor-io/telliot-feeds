...
@pytest.mark.asyncio
async def test_fetching_tips(tip_feeds_and_one_time_tips, chain):
    """Test fetching tips when there are both feed tips and single tips
    A one time tip and a feed tip should be returned"""
    flex = tip_feeds_and_one_time_tips
    datafeed, tip = await get_feed_and_tip(flex.autopay, skip_manual_feeds=False)
    assert isinstance(datafeed, DataFeed)
    assert isinstance(tip, int)
...
@pytest.mark.asyncio
async def test_fake_queryid_feed_setup(autopay_contract_setup, chain, caplog):
    """Test feed tips but no one time tips and no reported timestamps"""
    flex = autopay_contract_setup
...
@pytest.mark.asyncio
async def test_error_calculating_priceThreshold(autopay_contract_setup, chain, caplog):
...
@pytest.mark.asyncio
async def test_feed_with_manual_source(autopay_contract_setup, chain, caplog):
...
@pytest.mark.asyncio
async def test_no_value_before(autopay_contract_setup, chain, caplog):
    ...
    feed, tip = await get_feed_and_tip(r.autopay, skip_manual_feeds=False, current_timestamp=chain.pending_timestamp)
...
@pytest.mark.asyncio
async def test_low_gas_limit_error(autopay_contract_setup, chain, caplog):
...
@pytest.mark.asyncio
async def test_skip_manual_feed(autopay_contract_setup, chain, caplog):
