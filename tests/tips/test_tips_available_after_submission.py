...
async def test_feed_suggestion(autopay_contract_setup, caplog):
    ...
    query, tip_amount = await get_feed_and_tip(autopay=flex.autopay, skip_manual_feeds=False, current_timestamp=chain.time())
    ...
    tip_amount = await get_feed_and_tip(autopay=flex.autopay, skip_manual_feeds=False, current_timestamp=chain.time() + 5)
    assert tip_amount == (None, None)
    assert "No tips available in autopay" in caplog.text
    # bypass time until next window
    chain.sleep(3600)
    query, tip_amount = await get_feed_and_tip(autopay=flex.autopay, skip_manual_feeds=False, current_timestamp=chain.time() + 5)
    assert query.get_state()["query"] == {"type": "SpotPrice", "asset": "albt", "currency": "usd"}
    assert tip_amount == int(0.5 * 1e18)
    chain.sleep(43200)
